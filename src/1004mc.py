...
activeList = list(filter(isActive, listing)) #active listings
activeListMinPrice = min([x.ListingPrice for x in activeList]) if len(activeList) > 0 else 0
activeListMaxPrice = max([x.ListingPrice for x in activeList]) if len(activeList) > 0 else 0

soldList = list(filter(isSettled, listing))
soldListMinPrice = min([x.SellingPrice for x in soldList]) if len(soldList) > 0 else 0
soldListMaxPrice = max([x.SellingPrice for x in soldList]) if len(soldList) > 0 else 0
...
with open(outputFile, 'w') as outfile:
    ...
    outfile.write('\n\n')
    outfile.write("Active # Comp: {0}\tMin Price: {1}\tMaxPrice: {2}\n".format(len(activeList), activeListMinPrice, activeListMaxPrice))
    outfile.write("Sold   # Comp: {0}\tMin Price: {1}\tMaxPrice: {2}\n".format(len(soldList), soldListMinPrice, soldListMaxPrice))
